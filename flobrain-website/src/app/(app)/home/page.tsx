    ...
    <main className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-blue-500/30 overflow-hidden flex flex-col">
      ...
      <nav className="border-b border-white/5 p-4 flex justify-between items-center backdrop-blur-xl bg-slate-950/40 z-50">
        ...
        <div className="flex gap-4 items-center">
          <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900/80 border border-slate-800 shadow-inner">
            <div
              className={`w-2 h-2 rounded-full ${brainState === "processing" ? "bg-blue-500 animate-pulse shadow-[0_0_10px_#3b82f6]" : "bg-emerald-500 shadow-[0_0_8px_#10b981]"}`}
            />
            <span className="text-[9px] font-mono uppercase tracking-widest text-slate-200">
              Model: GPT-4o
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          <div
            className={`p-8 ${!leftPanelExpanded && "opacity-0 invisible"} transition-all duration-300`}
          >
              ...
              {["Initialize Core", "Load Memory Context", "Neural Path Sync"].map((step, i) => {
                ...
                return (
                  <div key={step} className="pl-8 relative group">
                    <div
                      className={`absolute -left-[6px] top-1 w-3 h-3 rounded-full border-2 transition-all duration-700 ${
                  ...
                  animate={
                    brainState === "processing"
                      ? {
                          ...
                          scaleY: [1, 1.3, 1],
                          filter: [
                            "drop-shadow(0 0 10px #3b82f688)",
                            "drop-shadow(0 0 0px #3b82f600)",
                          ],
                        }
                      : {}
                  }
        ...
        >
          ...
          <div
            className={`p-8 ${!rightPanelExpanded && "opacity-0 invisible"} transition-all duration-300`}
          >
            <h3 className="text-[10px] uppercase tracking-[0.25em] text-slate-500 mb-10 flex items-center gap-2 font-semibold">
            ...
            <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 min-h-[220px] shadow-inner relative overflow-hidden group hover:border-blue-500/20 transition-colors">
              <AnimatePresence mode="wait">
                {rightPanelView === "context" ? (
                  ...
                  >
                    <p className="text-[10px] font-mono text-blue-500/60 mb-4 tracking-widest uppercase">
                      {"\u002F\u002F"}Live_Stream
                    </p>
                    {brainState === "processing" ? (
                      ...
                      <p className="text-xs italic opacity-50 font-light font-mono text-slate-500">
                        {"\u002F\u002F"}Waiting for system trigger...
                      </p>
                    )}
                  </motion.div>
                ) : (
                  <motion.div
                    ...
                    className="min-h-[140px]"
                  >
                    <p className="text-[10px] font-mono text-emerald-500/60 mb-4 tracking-widest uppercase">
                      {"\u002F\u002F"}System_Logs
                    </p>
                    <div className="space-y-3 text-[10px] font-mono text-slate-500 uppercase">
                      <p className="flex justify-between">
                        <span>CPU_Usage</span> <span className="text-slate-300">12.4%</span>
                      </p>
                      <p className="flex justify-between">
                        <span>Memory_Link</span> <span className="text-emerald-400">Stable</span>
                      </p>
                      <p className="flex justify-between">
                        <span>Latency</span> <span className="text-slate-300">4ms</span>
                      </p>
                      <p className="flex justify-between">
                        <span>Uptime</span> <span className="text-slate-300">00:42:12</span>
                      </p>
                    </div>
                  </motion.div>
                )}
